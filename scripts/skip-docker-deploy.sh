...
# 设置 Node.js 内存限制到 1GB（降低限制，依赖交换空间）
export NODE_OPTIONS="--max-old-space-size=1024"
# 设置 Vite 构建时使用更少的并发任务
export NODE_ENV=production

print_info "使用内存限制: 1GB (配合交换空间)"
print_info "开始构建（可能需要较长时间，请耐心等待）..."

# 添加超时机制：如果 30 分钟内没有输出，提示用户
timeout 1800 pnpm build || {
    if [ $? -eq 124 ]; then
        print_error "构建超时（30分钟），可能因内存不足或网络问题"
        print_warn "建议："
        print_warn "1. 检查服务器内存和交换空间: free -h"
        print_warn "2. 查看构建进程: ps aux | grep -E 'node|vite|pnpm'"
        print_warn "3. 手动执行: cd ${REMOTE_PATH}/packages/frontend && NODE_OPTIONS='--max-old-space-size=1024' pnpm build"
        exit 1
    else
        print_error "构建失败"
        exit 1
